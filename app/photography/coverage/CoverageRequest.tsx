  ...
  return <div className="coverage-request">
    ...
    {!receipt && <form onSubmit={sendRequest} onChange={() => { setError(""); if (!started.current) { started.current = true; track("form_start"); } }}>
      ...
      <div className="coverage-fields">
        ...
        <p className="coverage-wide coverage-type-note" role="status">{kind === "volleyball" ? `One team’s varsity match: $350 for up to ${offer.onsiteHours} hours on site; 10 edited preview photos within 24 hours. ${offer.deliverySummary}` : kind ? "I’ll confirm a fixed total, photo deliverables and turnaround for your event before booking. The $350 volleyball package does not apply to this request." : "Choose the volleyball package or request a fixed quote for another sport or event."}</p>
