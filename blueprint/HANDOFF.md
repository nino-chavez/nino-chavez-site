...
**derive-corpus substrate** (2026-05-25, this commit + ADR-0006): the RAG corpus is now mechanically derived from source on every push to `main`. `tools/derive-corpus/derive-corpus.ts` reads the source list in `tools/derive-corpus/sources.ts` (19 sources currently: ADRs, blueprint docs, content drafts, voice guide, Poe stack, CLAUDE.md), chunks, embeds via OpenAI `text-embedding-3-small`, full-replaces in `askdad.content_chunks` per `source_id`. GitHub Actions workflow at `.github/workflows/ingest-askdad-corpus.yml` triggers on push to `main` and on `workflow_dispatch` only — **no schedule, no cron**, per prior Nino direction. Dry-run verified: ~55K tokens / ~$0.001 per full run. **Blocked on TWO items** (1Password-wired per global CLAUDE.md convention):

1. **`OP_SERVICE_ACCOUNT_TOKEN`** GitHub Actions repo secret — a 1Password service account token with read access to the "Developer Secrets" vault. One static secret; everything else (`OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`) is pulled at runtime via `op://Developer Secrets/...` references. Set via Settings → Secrets and variables → Actions.

2. **Update the `Supabase ask-dad` 1Password item's `url` field** to the cloud Supabase URL (currently set to `http://127.0.0.1:54521`, the local-dev URL — verified via `op item get`). The deployed corpus needs the cloud URL.

Once both land, the next push to main auto-populates the corpus. bc-subscriptions secrets are NOT reusable (different stack — no OpenAI / askdad-schema Supabase access). Companion docs: `blueprint/content/askdad-corpus-catalog.md` (human-readable), `tools/derive-corpus/README.md` (usage).
