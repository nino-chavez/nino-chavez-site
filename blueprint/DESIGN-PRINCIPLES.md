...
| `/practice` four sections (brief extension) | Brief originally specified 3 compositions: toolchain → operating-rules → instrumentation. Restructured to 4 sections after the "substrate" question surfaced: **01 The production line** (5 lathes per the canonical post) → **02 The substrate** (hooks + skills/subagents + session mining + Hive coordination) → **03 Operating rules** (declarative, mechanically enforced by 02) → **04 Instrumentation evidence** (hesitation fold + corpus stats + adversarial test plan). The original 3-section structure conflated the substrate with the instrumentation evidence and missed the agent/skill/Hive coordination layer entirely. Every claim in each section grounds to a real file or repo path (verified via `ls` and `gh repo view`). | ✓ Live. |
